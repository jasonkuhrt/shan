...
const encodeEntryToJson = Schema.encodeSync(Schema.parseJson(TranscriptEntrySchema, { space: 2 }))
...
export const transcriptDump = (input: string | undefined, options: DumpOptions = {}) =>
  Effect.gen(function* () {
    if (options.raw) {
      ...
      const sessionPath = input ? yield* resolveSessionPath(input) : yield* pickSession(options)
    ...
    const outputPath = yield* writeOutput(
      outputDir,
