...
const yamlQuote = (value: string): string =>
  /[:#\[\]{}&*!|>'"%@`,?]/.test(value) ? `"${value.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"` : value
...
export const skillsCreate = (name: string, options: SkillsCreateOptions) =>
  Effect.gen(function* () {
    if (!name || !VALID_NAME(name)) {
      ...
      yield* Console.error('  Each segment must start with a letter and contain only letters, digits, hyphens, or underscores')
      return yield* Effect.fail(new Error('Missing targets'))
    }

    const dir = Lib.outfitDir(options.scope)
    yield* Lib.ensureOutfitDir(dir)
    const relativePath = Lib.colonToPath(name)
    const skillDir = path.join(dir, relativePath)
