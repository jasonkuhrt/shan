 ...
 */
const resolveList = (lists: TaskListInfo[], identifier: string): TaskListInfo | null => {
  // 1. Exact match
...
export const resolveTarget = (input: string, options: ResolveOptions = {}) =>
  Effect.gen(function* () {
    ...
    if (!exists) {
      return yield* Effect.fail(new Error(`Task ${taskPart} not found in list ${list.id}`))
    }
...
const resolveBySubject = (lists: TaskListInfo[], query: string) =>
  Effect.gen(function* () {
    ...
    return yield* Effect.fail(new Error(`No task found with subject matching: ${query}`))
  })
